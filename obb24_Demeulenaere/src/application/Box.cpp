...
/** Projete les 4 sommets de la boite sur la direction axis. Donne l'intervalle [inf_,sup_] de la projection (inf_ devra être inférieur à sup_ pour la cohérence)
  - vertex(i) : donne la position du i-ème sommet de la boite (dans le repère du monde)
  - il faut déterminer le minimum (dans inf_) et le maximum (dans sup_) des projections des 4 sommets pour déterminer l'intervalle de la boite sur l'axe
**/
Interval Box::project(const Vector3 &axis) const {
  Interval res; // contient seulement des attributs inf_ et sup_ pour représenter l'intervalle [inf_,sup_]

  res.inf_=0.0;res.sup_=0.0;
  // A compléter : l'intervalle [inf_,sup_] doit correspondre à la projection de la boite sur l'axe
  double lambda0 = dot(vertex(0),axis);
  res.inf_= lambda0;
  res.sup_= lambda0;
  for (int i = 1; i< 4;i++){
      double lambda = dot(vertex(i),axis);
      res.inf_ = min(res.inf_,lambda);
      res.sup_ = max(res.sup_,lambda);
  }
