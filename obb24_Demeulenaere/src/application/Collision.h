...
public:
  ...
  DistanceInfo distance(const Box &b1, const Box &b2, const p3d::Vector3 &axis);
  void detectCollision(const Box &b1, const Box &b2);
  void detectCollision(const Box &b, const p3d::Plane &plane);

  static void drawDebugProjection(const Box &b1,const Box &b2,const p3d::Vector3 &axis,Interval i1,Interval i2);
