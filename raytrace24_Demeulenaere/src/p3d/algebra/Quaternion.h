...
public:
  ...
  /// @brief this is composed with the rotation (angle in degrees)
  Quaternion &rotate(double angle,double x,double y,double z);
  /// @return the rotation matrix
  Matrix4 toMatrix();
  /// @brief apply the quaternion (i.e. the rotation) to vector u
  void transform(Vector3 &u) const;
  /// @brief return inverse of the quaternion (inverse of the rotation)
  Quaternion inverse();
  ...
  static Quaternion fromAngleAxis(double angle,double x,double y,double z);
  ...
  static Quaternion fromDirection(const Vector3 &v1,const Vector3 &v2);
