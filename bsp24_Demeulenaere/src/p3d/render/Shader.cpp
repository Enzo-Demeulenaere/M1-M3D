...
void Shader::uniform(const string &nom,const vector<float> &t) {
  int loc=uniform(nom+"[0]");
  uniform(loc,t);
}

void Shader::uniform(int loc,const vector<float> &t) {
  glUniform1fv(loc,static_cast<GLsizei>(t.size()),t.data());
}
...
void Shader::uniform(const string &name,const vector<Vector2> &v) {
  int loc=uniform(name+"[0]");
  uniform(loc,v);
}
...
void Shader::uniform(int loc,const vector<Vector2> &v) {
  vector<float> mv;
  mv.resize(v.size()*2);
  size_t i=0,j=0;
  for(auto &val:mv) {
  ...
  glUniform2fv(loc,static_cast<GLsizei>(v.size()),mv.data());
}
