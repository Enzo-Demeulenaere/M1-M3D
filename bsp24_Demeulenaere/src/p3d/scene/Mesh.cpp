...
const Vector3 &Mesh::positionMesh(int i) const {return positionMesh_[i];}
const Vector3 &Mesh::normalMesh(int i) const {return normalMesh_[i];}
const Vector2 &Mesh::texCoordMesh(int i) const {return texCoordMesh_[i];}

const Vector3 &Mesh::normalFace(int i) const {return normalFace_[i];}

const VertexAttrib &Mesh::vertexAttrib(int i,int j) const {return vertexAttrib_[i][cycle(j,nbVertexFace(i))];}
int Mesh::indexPositionVertexFace(int i,int j) const {return vertexAttrib(i,j).positionIndex_;}
const Vector3 &Mesh::positionVertexFace(int i,int j) const {return positionMesh(indexPositionVertexFace(i,j));}
const Vector3 &Mesh::normalVertexFace(int i,int j) const {return normalMesh(vertexAttrib(i,j).normalIndex_);}
const Vector2 &Mesh::texCoordVertexFace(int i,int j) const {return texCoordMesh(vertexAttrib(i,j).texCoordIndex_);}

int Mesh::nbPosition() const {return positionMesh_.size();}
int Mesh::nbNormal() const {return normalMesh_.size();}
int Mesh::nbTexCoord() const {return texCoordMesh_.size();}
int Mesh::nbVertexFace(int i) const {return vertexAttrib_[i].size();}
int Mesh::nbFace() const {return vertexAttrib_.size();}
...
void Mesh::computeNormalFace(int i) {
  Vector3 s1;
  Vector3 s2;
  Vector3 s3;
...
void Mesh::computeNormal() {
  ...
  for(int i=0;i<nbFace();++i) {
    for(int j=0;j<nbVertexFace(i);++j) {
...
void Mesh::triangulate() {
  ...
  for(int i=0;i<nb;i++) {
    if (nbVertexFace(i)>3) {
      ...
      for(int j=0;j<nbVertexFace(i)-3;++j) {
        add.clear();
...
//  trans.translate(-(_aabbMax+_aabbMin)/2.0);
  for(int i=0;i<int(positionMesh_.size());++i) {
