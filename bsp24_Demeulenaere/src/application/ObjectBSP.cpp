...
NodeBSP *ObjectBSP::consTree(const std::vector<FaceBSP> &face) {
  ...
  if (face.empty()) {
    ...
    res->face(pivot);
    for (int i=1; i<int(face.size());i++){
        FaceBSP currentFace = face[i];
        pivot.separe(currentFace,fPositive,fNegative);
        if(not fPositive.empty()) positive.push_back(fPositive);
        if(not fNegative.empty()) negative.push_back(fNegative);
    }
...
void ObjectBSP::drawBSP(NodeBSP *tree,const Vector3 &eye) {
  ...
  FaceBSP face = tree->face();

  if(not face.empty()){
      if(face.sign(eye) == Sign_Plus){
...
void ObjectBSP::read(const string &resourceName, p3d::Vector3 mini,p3d::Vector3 maxi) {
//  QFileInfo name=resourceFile(resourceName);
  vector<VertexBSP> allVertex;
  Mesh o;
  o.read(resourceName);
  o.scaleInBoxRatio(mini.x(),maxi.x(),mini.y(),maxi.y(),mini.z(),maxi.z());
  o.computeNormal();

  for(int i=0; i<o.nbPosition(); ++i) {
    allVertex.push_back(VertexBSP(o.positionMesh(i),o.normalMesh(i)));
  }
  for(int i=0; i<o.nbFace(); ++i) {
    FaceBSP f;
    f.normal(o.normalFace(i));
    for(int j=0; j<o.nbVertexFace(i); ++j) {
