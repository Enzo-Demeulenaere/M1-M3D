...
Quaternion Quaternion::fromDirection(const Vector3 &v1,const Vector3 &v2) {
  Quaternion q;
...
Quaternion &Quaternion::rotate(double angle,double x,double y,double z) {
  return rotate(AngleAxis(angle,Vector3(x,y,z)));
}
...
Quaternion Quaternion::fromAngleAxis(double angle,double x,double y,double z) {
  return AngleAxis(angle,x,y,z).toQuaternion();
}
