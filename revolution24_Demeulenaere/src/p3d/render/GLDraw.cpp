...
void p3d::drawLineStrip(const std::vector<Vector2> &point) {
  drawLineStrip(point,point.size());
}
...
VertexArray* p3d::fromGridLine(const std::vector<Vector3> g,int nbU,int nbV) {
  VertexArray* res=new VertexArray(GL_LINES);
  res->add("position",g,0);
  vector<GLuint> index;
  int nb = 4*nbU*nbV-2*(nbU+nbV);
  index.resize(nb);
  int current = 0;
  for(int j=0;j<nbV;++j) {
    int k=j*nbU;
...
//VertexArray* p3d::fromBigPoint(const std::vector<Vector3> p,const std::vector<Vector4>)
