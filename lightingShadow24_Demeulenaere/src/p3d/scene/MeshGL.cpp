...
void MeshGL::drawLine(double scale) {
  ...
  for(int i=0;i<mesh_->nbFace();++i) {
    Vector3 g(0,0,0);
    for(int j=0;j<mesh_->nbVertexFace(i);++j) {
...
void MeshGL::drawNormal(double kLength) {
  if (!mesh_) return;
    ...
    for(int i=0;i<mesh_->nbFace();++i) {
        for(int j=0;j<mesh_->nbVertexFace(i);++j) {
            pts.push_back(mesh_->positionVertexFace(i,j));
            pts.push_back(mesh_->positionVertexFace(i,j)+kLength*mesh_->normalVertexFace(i,int(j)));
        }
    }
