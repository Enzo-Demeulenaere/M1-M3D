...
GLApplication::GLApplication() {
  ...
  leftPanelMenu_ << "Show/Hide Control Points";
...
void GLApplication::update() {
  ...
  if (keyPressed(Qt::Key_C)) {
    whichInteraction_->clear();
  }

  if (setCatmullRom_) catmullRom_.setup();
...
void GLApplication::leftPanel(size_t i,const std::string &s) {
  ...
  case 2:modeDraw_=Draw_Catmull;whichInteraction_=&catmullRom_;setCatmullRom_=false;break;
  case 3:modeDraw_=Draw_Catmull;whichInteraction_=&catmullRom_;setCatmullRom_=true;break;
...
void GLApplication::printMode() {
  ...
  case Draw_Catmull:
    if (setCatmullRom_) {
      ...
      p3d::draw("Cubic Interpolation",Vector2(-1,-1));
    }
    break;
  case Draw_Hermite:
